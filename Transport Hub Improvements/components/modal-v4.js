...
const PanelManager = {
    ...
    renderPanel() {
        ...
        document.getElementById('panelBody').innerHTML = `
            ...
            <!-- Support row -->
            <div class="pnl-support-divider"></div>
            <div class="pnl-support-row">
                <span class="material-symbols-outlined pnl-support-icon">support_agent</span>
                <span class="pnl-support-text">For support call <a class="pnl-support-phone">(716) 431-3800.</a></span>
            </div>

            <!-- Cancel -->
            <button class="pnl-cancel-btn">Request Cancellation</button>
        `;
        ...
        stages.forEach((label, idx) => {
            ...
            const timeVisible = completed || active;
            const dotSrc = allDone
                ? 'assets/step-completed.svg'
                : completed
                    ? 'assets/step-done.svg'
                    : active
                        ? 'assets/step-inprogress.svg'
                        : 'assets/step-default.svg';
            stepperParts.push(`
                <div class="pnl-step-col${active ? ' active' : ''}">
                    <img src="${dotSrc}" alt="" class="pnl-step-dot-img" />
                    <div class="pnl-step-label">${label}</div>
                    <div class="pnl-step-time${timeVisible ? '' : ' pnl-step-time-hidden'}">${v.progressTimestamps[idx].time}</div>
                </div>`);
